...
cd node-can
pnpm install        # builds native addon + compiles TypeScript
```

## Building

```sh
pnpm run build:all  # native addon + TypeScript
...
This runs `pnpm install` inside a `node:22-bookworm-slim` container and validates the native build and TypeScript compilation.
...
Tests import from `../dist/socketcan`, so always run `pnpm run build:ts` after changing TypeScript source before running tests.
...
4. Run `pnpm run lint` and `pnpm test` — both must pass.
...
**TypeScript:** Prettier + ESLint — run `pnpm run lint` to auto-fix. Use `===`, named constants over magic numbers.
